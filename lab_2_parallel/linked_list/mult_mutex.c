...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      // Operación de búsqueda (Member)
      if (which_op < search_percent) {
         GET_TIME(start_mutex);
         Member(val);
         GET_TIME(end_mutex);
         my_member++;
         total_mutex_time += (end_mutex - start_mutex);  // Tiempo de espera por el mutex
      } 
   ...
   total_thread_time = end_thread - start_thread;  // Tiempo total de ejecución del hilo

   // Actualizamos los contadores globales con mutex de protección
   ...
   printf("Hilo %ld - Member: %d, Insert: %d, Delete: %d\n", my_rank, my_member, my_insert, my_delete);
